...
def choose_and_visualize():
    ...
    last_file_list.sort()
    chosen_texture = st.sidebar.selectbox("Choose the Motion-Texture Pair", last_file_list)
    last_path = os.path.join(last_path, chosen_texture)
    last_file_list = os.listdir(last_path)
    last_file_list.sort()
    chosen_exp = st.sidebar.selectbox("Choose the Experiment Setting", last_file_list)

    last_path = os.path.join(last_path, chosen_exp)
